...
import {createCheckIn, createRegistration, loadCheckInsForDay, loadChildByQrId, loadChildren, loadParents, loadRegistrations, loadRegistrationsForDay, updateInfo} from "./sheet";
...
function getRequest(e: GoogleAppsScript.Events.DoGet) {
    ...
    switch (endpoint) {
        ...
        case "parent": {
            const qrId: string = e.parameter["code"];

            const child: Child = loadChildByQrId(qrId);
            return returnJSON({
                ...loadParents(child),
                child: child
            });
